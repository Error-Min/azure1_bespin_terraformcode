...
resource "azurerm_mysql_flexible_server" "smlee_mysql" {#
  name                = "smlee-mysql"
  location            = azurerm_resource_group.smlee_rg.location
  resource_group_name = azurerm_resource_group.smlee_rg.name
  administrator_login          = var.admin_user
  administrator_password      = var.admin_password
  delegated_subnet_id    = azurerm_subnet.smlee_db_subnet.id
  private_dns_zone_id    = azurerm_private_dns_zone.smlee_private_dns_zone.id
  sku_name   = "GP_Standard_D2ds_v4"
  depends_on = [azurerm_private_dns_zone_virtual_network_link.smlee_dns_link]
}
...
resource "azurerm_mysql_flexible_server_configuration" "time_zone" {#
  name                = "time_zone"
  resource_group_name = azurerm_resource_group.smlee_rg.name
  server_name         = azurerm_mysql_flexible_server.smlee_mysql.name
  value               = "-09:00"
}
...
/*resource "azurerm_private_endpoint" "smlee_private_endpoint" {
    ...
    private_service_connection {
        ...
        private_connection_resource_id = azurerm_mysql_flexible_server.smlee_mysql.id
